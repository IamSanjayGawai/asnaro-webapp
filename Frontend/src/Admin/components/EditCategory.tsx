...
import { updateCategoryThunk } from "@/state/thunks/adminThunks";
import { useAppDispatch } from "@/state/hooks";

interface DataProps {
  isEditCatModal: boolean;
  setIsEditCatModal?: (showModal: boolean) => void;
  show?: boolean;
...
const Modal = ({
  ...
  const [category, setCategory] = useState(updateCatName);
  const dispatch = useAppDispatch();

  const handleCreate = () => {
    dispatch(updateCategoryThunk({ category_name: category, id }));
  };
              ...
              <div className="border-0  shadow-lg relative flex gap-5 flex-col w-full bg-white outline-none focus:outline-none">
                ...
                <div className="flex items-start justify-between p-4  rounded-t">
                  <button
                    ...
                    onClick={() => setIsEditCatModal(false)}
                  >
                    <RxCross2 />
                  </button>
                </div>
