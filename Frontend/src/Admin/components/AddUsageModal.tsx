...
import { createusagefeeMasterThunk } from "@/state/thunks/adminThunks";
...
const Modal = ({ showModal, setShowModal, show = true }: DataProps) => {
  const [sent] = useState(true); // default send to true
  const dispatch = useAppDispatch();
  const [usage_name, setUsageName] = useState("");
  ...
  const handleCreateUsageFee = async () => {
    ...
    await dispatch(createusagefeeMasterThunk(usageData));
    setShowModal(false);
  };
