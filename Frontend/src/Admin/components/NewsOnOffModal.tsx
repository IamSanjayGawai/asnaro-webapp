...
import { newsTurnOffThunk } from "@/state/thunks/adminThunks";
...
interface DataProps {
  ...
  idToTurnOnOff?: string;
  delievryTurnOnOff?: boolean;
}
...
const Modal = ({
  ...
  const handleSubmit = async () => {
    await dispatch(newsTurnOffThunk(idToTurnOnOff));
    socket.emit("newsToggle", { newsId: idToTurnOnOff }, () => {
      setShowModal(false);
    });
  };
                ...
                {!sent ? (
                  <div className="relative px-4 gap-5 flex-auto ">
                    ...
                    <p className="text-[#808080] text-sm font-noto-sans-jp px-8 text-base font-[500]  w-2/3 mt-[4em] mb-[3em] mx-auto">
                      {`このニュースを送信することに同意しますか ${delievryTurnOnOff ? "非公開" : "公開"} ?

「はい」の場合は、「適用」ボタンをクリックします。 「いいえ」の場合は、「戻る」をクリックします。`}
                    </p>
                  </div>
                ) : (
                  <div className="relative px-4 gap-5 flex-auto ">
