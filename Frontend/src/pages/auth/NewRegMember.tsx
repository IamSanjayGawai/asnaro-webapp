...
import { userRegisterThunk } from "@/state/thunks/userThunks";
...
const NewRegMember: React.FC<RegisterProps> = ({ setAuth }) => {
  ...
  const { user, registerErr } = useAppSelector(selectUser);
  ...
  React.useEffect(() => {
    if (user?.message === "User created successfully" && redirect) {
          ...
          <div className="grid grid-cols-12 gap-y-3 gap-x-5">
            ...
            <div className="grid grid-cols-24 col-span-12 xs:gap-y-2 md:gap-y-">
              ...
              >
                <input
                  className={`h-[36px] w-full outline-none border ${
                    errors.email || registerErr?.message
                      ? "border-fifth"
                      : "border-[#E6E6E6]"
                  } placeholder-[#E6E6E6] px-1`}
                  type="text"
                ...
                {registerErr && (
                  <p className="text-red-600">{registerErr?.message}</p>
                )}
              </div>
            </div>
            <div className="grid grid-cols-24 col-span-12 xs:gap-y-2 md:gap-y-">
