...
import { processAddThunk } from "@/state/thunks/processThunks";
...
const ProcessReg: React.FC = () => {
  ...
  const { process, appErr } = useAppSelector(selectProcess);
  ...
  React.useEffect(() => {
    if (process?.message === "Process added successfully" && redirect) {
  ...
  const onSubmit: SubmitHandler<ProcessRegForm> = async (data) => {
    ...
    await dispatch(processAddThunk(formData));
  ...
  return (
    <>
      {loading ? (
        ...
        <div className="px-[45px] xl:px-0 xl:pl-[50px] xl:pr-[70px] md:max-w-[800px] mx-auto w-full text-base text-tertiary">
          <form
            className="form"
            ...
            id="processForm"
          >
            <div className="grid grid-cols-12 xs:gap-y-2 md:gap-y-3">
              ...
              {(errors.img1 || errors.img2 || errors.img2) && (
                <div className="text-fifth grid grid-cols-12 col-span-12">
                  <div className="xs:hidden md:block col-span-3"></div>
                  <p className="xs:col-span-12 md:col-span-9">
                    3 つの画像フィールドはすべて必須です
