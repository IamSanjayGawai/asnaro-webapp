...
const SellerTransaction = () => {
  ...
  const [socketEstimatePdf, setSocketEstimatePdf] = useState<string>("");
  const [socketAdvancePaymentPdf, setSocketAdvancePaymentPdf] =
    useState<string>("");
  const [socketOrderFormPdf, setSocketOrderFormPdf] = useState<string>("");
  const [socketDeliverySlipPdf, setSocketDeliverySlipPdf] =
    useState<string>("");
  const [socketReceiptPdf, setSocketReceiptPdf] = useState<string>("");
  const [socketAcceptancePdf, setSocketAcceptancePdf] = useState<string>("");
  const [socketTransactionInvoicePdf, setSocketTransactionInvoicePdf] =
    useState<string>("");
  const [socketSystemFeePdf, setSocketSystemFeePdf] = useState<string>("");
  const [socketRefundPdf, setSocketRefundPdf] = useState<string>("");
  const [
    socketRefundTransactionInvoicePdf,
    setSocketRefundTransactionInvoicePdf,
  ] = useState<string>("");
  const [socketRefundSystemFeePdf, setSocketRefundSystemFeePdf] =
    useState<string>("");
  const [socketTransactionForPdf, setSocketTransactionForPdf] = useState();
  const [socketTransactionForAdvance, setSocketTransactionForAdvance] =
    useState();
  const [socketTransactionForOrder, setSocketTransactionForOrder] = useState();
  const [
    socketTransactionForDeliverySlip,
    setSocketTransactionForDeliverySlip,
  ] = useState();
  const [socketTransactionForReceipt, setSocketTransactionForReceipt] =
    useState();
  const [socketTransactionForAcceptance, setSocketTransactionForAcceptance] =
    useState();
  const [socketTransactionForInvoice, setSocketTransactionForInvoice] =
    useState();
  const [socketTransactionForSystem, setSocketTransactionForSystem] =
    useState();
  const [socketTransactionForRefundPdf, setSocketTransactionForRefundPdf] =
    useState();
  const [
    socketTransactionForRefundTransactionInvoice,
    setSocketTransactionForRefundTransactionInvoice,
  ] = useState();
  const [
    socketTransactionForRefundSystem,
    setSocketTransactionForRefundSystem,
  ] = useState();
  ...
  const taxRate =
    transaction?.transaction?.adminTax !== 0
      ? transaction?.transaction?.adminTax
      : taxData && taxData.tax
      ? taxData.tax
      : 10;
  const refundTaxRate =
    transaction?.transaction?.refund?.taxDetails?.adminRefundTax !== 0
      ? transaction?.transaction?.refund?.taxDetails?.adminRefundTax
      : taxData && taxData.tax
      ? taxData.tax
      : 10;
  const hasRun = useRef<boolean>(false);
  const isBlueButtonForEstimation = socketEstimatePdf
    ? socketEstimatePdf
    : transaction && transaction?.transaction?.estimation_pdf;
  const isBlueButtonForAdvancePayment = socketAdvancePaymentPdf
    ? socketAdvancePaymentPdf
    : transaction && transaction?.transaction?.advance_payment_invoice;
  const isBlueButtonForOrderForm = socketOrderFormPdf
    ? socketOrderFormPdf
    : transaction && transaction?.transaction?.order_form;
  const isBlueButtonForDeliverySlip = socketDeliverySlipPdf
    ? socketDeliverySlipPdf
    : transaction && transaction?.transaction?.delivery_slip;
  const isBlueButtonForReceipt = socketReceiptPdf
    ? socketReceiptPdf
    : transaction && transaction?.transaction?.Receipt;
  const isBlueButtonForAcceptanceLetter = socketAcceptancePdf
    ? socketAcceptancePdf
    : transaction && transaction?.transaction?.acceptance_letter;
  const isBlueButtonForTransactionInvoice = socketTransactionInvoicePdf
    ? socketTransactionInvoicePdf
  ...
  const isBlueButtonForSystemFee = socketSystemFeePdf
    ? socketSystemFeePdf
    : transaction && transaction?.transaction?.system_fee_invoice;
  const isBlueButtonForRefundSystemFee = socketRefundSystemFeePdf
    ? socketRefundSystemFeePdf
  ...
  useEffect(() => {
    ...
    socket.on("newMessageQuote", (data) => {
      data.estimatePdf && setSocketEstimatePdf(data.estimatePdf);
      data.transaction && setSocketTransactionForPdf(data.transaction);
    ...
    socket.on("newMessagePayment", (data) => {
      data.advancePaymentPdf &&
        setSocketAdvancePaymentPdf(data.advancePaymentPdf);
      data.orderFormPdf && setSocketOrderFormPdf(data.orderFormPdf);
      data.transaction && setSocketTransactionForAdvance(data.transaction);
      data.transaction && setSocketTransactionForOrder(data.transaction);
      setPayByBuyer(true);
      setPaymentDone(true);
      dispatch(setNewNotification(true));
      setChatArray((prevChatArray) => [
    ...
    socket.on("newMessageDeliveryCompleted", (data) => {
      data.deliverySlipPdf && setSocketDeliverySlipPdf(data.deliverySlipPdf);
      data.receiptPdf && setSocketReceiptPdf(data.receiptPdf);
      data.transaction && setSocketTransactionForDeliverySlip(data.transaction);
      data.transaction && setSocketTransactionForReceipt(data.transaction);
    ...
    socket.on("newMessageDeliveryAccepted", (data) => {
      ...
      data.acceptancePdf && setSocketAcceptancePdf(data.acceptancePdf);
      data.transactionInvoicePdf &&
        setSocketTransactionInvoicePdf(data.transactionInvoicePdf);
      data.systemFeePdf && setSocketSystemFeePdf(data.systemFeePdf);
      data.transaction && setSocketTransactionForAcceptance(data.transaction);
      data.transaction && setSocketTransactionForInvoice(data.transaction);
      data.transaction && setSocketTransactionForSystem(data.transaction);
      setDeliveryOK(true);
      setPayByBuyer(false);
      dispatch(setNewNotification(true));
      setChatArray((prevChatArray) => [
    ...
    socket.on("newMessageRequestRefund", (data) => {
      ...
      data.refundPdfLink && setSocketRefundPdf(data.refundPdfLink);
      data.transaction && setSocketTransactionForRefundPdf(data.transaction);
    ...
    socket.on("newMessageRefundTermsAgree", (data) => {
      handleReviewModalShow();
      data.refundTransactionInvoicePdf &&
        setSocketRefundTransactionInvoicePdf(data.refundTransactionInvoicePdf);
      data.refundSystemFeePdf &&
        setSocketRefundSystemFeePdf(data.refundSystemFeePdf);
      data.transaction &&
        setSocketTransactionForRefundTransactionInvoice(data.transaction);
      data.transaction && setSocketTransactionForRefundSystem(data.transaction);
    ...
    socket.on("adminPaymentStatusChangeRefresh", async (data) => {
      console.log("adminPaymentStatusChangeRefresh", data);
      setAdminRefresh(true);
      if (data.buyerId === transaction?.user?.id) {
  ...
  const fetchData = async () => {
    ...
    await dispatch(getTaxThunk({}));
    await dispatch(getUpperLimitsThunk({}));
  };
  ...
  const handleDownload = () => {
    if (socketTransactionForPdf) {
      DownloadPDF(socketTransactionForPdf);
    } else {
      DownloadPDF(transaction);
    }
  };
  ...
  const handleDownloadAdvancePayment = () => {
    if (socketTransactionForAdvance) {
      DownloadPDFAdvancePayment(socketTransactionForAdvance);
    } else {
  ...
  const handleDownloadOrderForm = () => {
    if (socketTransactionForOrder) {
      DownloadPDFOrderForm(socketTransactionForOrder);
    } else {
      DownloadPDFOrderForm(transaction);
    }
  };
  ...
  const handleDownloadDeliverySlip = () => {
    if (socketTransactionForDeliverySlip) {
      console.log("reached here");
      DownloadPDFDelivery(socketTransactionForDeliverySlip);
    } else {
      DownloadPDFDelivery(transaction);
    }
  };
  ...
  const handleDownloadReceipt = () => {
    if (socketTransactionForReceipt) {
      DownloadPDFReceipt(socketTransactionForReceipt);
    } else {
      DownloadPDFReceipt(transaction);
    }
  };
  ...
  const handleDownloadAcceptance = () => {
    if (socketTransactionForAcceptance) {
      DownloadPDFAcceptance(socketTransactionForAcceptance);
    } else {
      DownloadPDFAcceptance(transaction);
    }
  };
  ...
  const handleDownloadTransactionInvoice = () => {
    if (socketTransactionForInvoice) {
      DownloadPDFTransactionInvoice(socketTransactionForInvoice);
    } else {
  ...
  const handleDownloadRefundInvoice = () => {
    if (socketTransactionForRefundPdf) {
      DownloadPDFRefundInvoice(socketTransactionForRefundPdf);
    } else {
      DownloadPDFRefundInvoice(transaction);
    }
  };
  const handleDownloadRefundTransactionInvoice = () => {
    if (socketTransactionForRefundTransactionInvoice) {
      DownloadPDFRefundTransactionInvoice(
        socketTransactionForRefundTransactionInvoice
      );
    } else {
  ...
  const handleDownloadSystem = () => {
    if (socketTransactionForSystem) {
      DownloadPDFSystem(socketTransactionForSystem);
    } else {
  ...
  const handleDownloadRefundSystem = () => {
    if (socketTransactionForRefundSystem) {
      DownloadPDFRefundSystem(socketTransactionForRefundSystem);
    } else {
  ...
  // console.log("deadline", compactTableData?.nodes[0]?.deadline);
  // console.log(
  ...
  // console.log("upper limits", upperLimitAmount);
  // console.log("socketTransactionForPdf", socketTransactionForPdf);
  ...
  return (
    <>
      {loading ? (
        ...
        <div className="flex justify-center flex-col  items-center sm:mb-80 mb-10  text-start">
          {/* main div */}
          <div className="md:block max-w-[1200px] w-full sm:px-6 md:px-8 xs:py-3 md:py-5 lg:py-6 flex justify-start flex-col xs:px-[1.8rem] ">
            {transaction &&
            transaction?.user &&
            transaction?.user?.name &&
            transaction?.user.name === transaction?.seller_name ? (
              <p
                className=" text-[#808080] lg:text-[18px] xs:text-[12] cursor-pointer lg:w-1/3"
                onClick={() => {
                  router(
                    `/process/company-profile/${transaction?.transaction?.seller_id?._id}`
                  );
                }}
              >
            ...
            transaction?.user.name === transaction?.seller_name ? (
              <p
                className="text-[#255BB3] lg:text-[24px] xs:text-[18px] font-bold mt-2 cursor-pointer lg:w-1/3"
                onClick={() => {
                  router(
                    `/process/company-profile/${transaction?.transaction?.seller_id?._id}`
                  );
              ...
              <div className="w-full mt-8">
                ...
                <div className="flex xs:flex-col lg:flex-row justify-between border p-4 mt-2">
                  <div className="lg:w-[8rem] xs:h-[5rem] flex lg:justify-center lg:items-center xs:justify-start xs:items-center ">
                    ...
                    />
                    <span
                      className="text-[#255BB3] lg:hidden xs:block font-bold xs:ml-5 lg:text-[18px] xs:text-[16px] cursor-pointer"
                      onClick={() => {
                        router(
                          `/process/company-profile/${transaction?.transaction?.customer_id?._id}`
                        );
                      }}
                    >
                  ...
                  <div className="flex-1 flex flex-col lg:ml-3 ">
                    <span
                      className="text-[#255BB3] lg:block xs:hidden font-bold lg:text-[18px] xs:text-[16px] cursor-pointer"
                      onClick={() => {
                        router(
                          `/process/company-profile/${transaction?.transaction?.customer_id?._id}`
                        );
            ...
            {!transaction?.transaction?.initiateRefund?.IsTrue ? (
              <div className="mt-4 border-[1px] border-[#E6E6E6] overflow-x-auto">
                ...
                <div className="w-full bg-[#fff] text-[#808080] py-1  gap-5 flex items-center lg:justify-end xs:justify-start lg:text-[16px] xs:text-[14px] pr-3 ">
                  <span className="font-bold bg-[#FFF4DF] p-2 px-[16px] text-[#808080] lg:text-[16px] ">
                    納期
                  </span>
                  {compactTableData?.nodes[0]?.deadline}
                </div>
                ...
                <div className="bg-[#ffffff] lg:w-full xs:w-[200%]  h-[2rem] lg:text-[18px] xs:text-[14px] flex items-center justify-end">
                  <div className="flex justify-between w-1/4">
                    <span className="text-[#808080]">消費税({taxRate}%)</span>{" "}
                    <span className="mr-3 text-[#808080]">
                      {socketEstimate &&
                      backQuotation &&
                      socketEstimate?.length > 0
                ...
                <div className="bg-[#ffffff] lg:w-full  xs:w-[200%]  h-[5rem] lg:text-[18px] xs:text-[14px] flex items-center justify-start">
                  <div className="w-[6rem] h-[5rem] bg-[#E6E6E6] flex items-center justify-center text-[#808080]">
                    特記事項
                  </div>{" "}
            ...
            {transaction?.transaction?.transaction_status !== 9 && (
              <div className=" w-full flex   gap-2 mt-8 cursor-pointer">
                <div
                  className={`flex   ${
                    transaction && transaction?.transaction?.paymentDeposit
