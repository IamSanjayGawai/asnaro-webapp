...
type Props = {
  ...
  process?: ProcessContainer | null;
};

function SearchProcessCards({ grid, process }: Props) {
  const router = useNavigate();
  const dispatch = useAppDispatch();
  const [image, setImage] = useState(process?.process?.img1);
  const handleNavigate = () => {
    router(`/process/details/${process?.process?._id}`);
  };

  console.log("Process in search", process);
  ...
  return (
    ...
    >
      <div>
        ...
        >
          <img
            src={process?.process?.img1}
            alt="ProcessImg"
            onClick={() => setImage(process?.process?.img1)}
            className="cursor-pointer"
            onError={handleImageError}
          />
          <img
            src={process?.process?.img2}
            onClick={() => setImage(process?.process?.img2)}
            alt="ProcessImg"
            className="cursor-pointer"
            onError={handleImageError}
          />
          <img
            src={process?.process?.img3}
            onClick={() => setImage(process?.process?.img3)}
            alt="ProcessImg"
            className="cursor-pointer"
            onError={handleImageError}
          />
      ...
      <div className={grid ? "sm:px-0 px-4 mt-4" : "p-4"}>
        ...
        >
          <h1
            ...
            onClick={() =>
              router(`/process/company-profile/${process?.process?.user?._id}`)
            }
          >
            {process?.process?.user?.name01}
          </h1>

          <div className="flex lg:gap-2 xs:gap-1 items-center ">
            <svg
            ...
            <h1 className="sm:text-[16px] text-[12px] text-[#808080] font-[400] ">
              {`${process?.process?.pref ? process?.process?.pref + ", " : ""}${
                process?.process?.mun ? process?.process?.mun : ""
              }`}
            </h1>
          </div>
        </div>
        <div className=" grid grid-cols-1 gap-2 sm:pb-0 pb-4 w-full sm:w-fit mt-2">
          <RatingSystem
            averageRating={process?.process?.totalRatingSum || 0}
            totalRatings={process?.process?.totalReviews || 0}
          />
          ...
          >
            {process?.process?.tags &&
              process?.process?.tags?.map((tag, index) => (
                <h2
                  key={index}
                  className={`text-[12px] ${
        ...
        >
          ...
          <h2 className="sm:text-[14px] text-[12px] text-[#808080] font-[400] w-4/5 line-clamp-3">
            {process?.process?.process_explanation}
          </h2>
        </div>
        ...
        >
          ...
          <h2 className="sm:text-[14px] text-[12px] text-[#808080] font-[400] w-4/5 line-clamp-2">
            {process?.process?.capacity}
          </h2>
        </div>
        ...
        >
          ...
          <h2 className="sm:text-[14px] text-[12px] text-[#808080] font-[400] w-4/5  line-clamp-1 ">
            {process?.process?.delivery_date}
          </h2>
        </div>
        <div
          className={`flex items-center gap-3 mt-4  bg-[#FFF4DF] w-full  py-2 px-2 ${
            grid ? "hidden sm:flex" : "flex"
          }`}
        >
          <h2 className="bg-[#FFAA00] text-center font-[500] p-1 sm:text-[14px] text-[12px] text-[#fff] w-[80px] py-1">
            価格
          </h2>
          <div className="flex  items-center py-2  w-full">
            <h2 className="sm:text-[14px] text-[14px] text-[#FFAA00] font-[400] w-4/5  line-clamp-1 ">
      ...
      <div className={"w-full px-2 pb-2 sm:hidden"}>
        <button
          onClick={handleNavigate}
          className="bg-[#255BB3] text-center w-full text-[#fff] p-2 flex items-center justify-center gap-2 font-bold"
        >
          空き状況 <FaAngleRight />
        </button>
      </div>
