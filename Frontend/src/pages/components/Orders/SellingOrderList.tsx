...
const BuyingOrderList = () => {
  ...
  const { sellerList, transactionStatus } = useAppSelector(selectTransaction);
  ...
  const startPage = Math.max(
    1,
    sellerList?.pagination?.currentPage - Math.floor(maxButtons / 2)
  );
  const endPage = Math.min(
    sellerList?.pagination?.totalPages,
  ...
  const handlePageChange = async (pageNumber) => {
    ...
    navigate(`/dashboard/order-list?page=${pageNumber}`);
  };

  useEffect(() => {
    dispatch(getSellerListThunk({ status: filter, page: currentPage }));
    dispatch(getTransactionStatus({ status: filter, role: "seller" }));
  ...
  let nodes = [];
  if (sellerList && sellerList?.data) {
    for (let i = 0; i < sellerList?.data.length; i++) {
      const item = sellerList?.data[i];
      nodes.push({
        ...
        company_name: item.sellerDetails[0].name01,
  ...
  const COLUMNS = [
    ...
    },
    {
      ...
      renderCell: (cell) => (
        <div
          className="text-[#808080] text-[14px] font-[500] w-fit bg-[#E6E6E6] py-1  text-center mx-auto cursor-pointer"
          style={{ whiteSpace: "normal" }}
          onClick={() => {
            if (cell.paymentDeposit) {
              if (cell.transaction_status === 5) {
      ...
      {loading ? (
        ...
        <div className="mx-8 mt-[-0.5rem]">
          ...
          <div className="col-span-12 bg-[#F5F5F5] h-[60px] flex justify-center items-center gap-2 mt-[50px]">
            ...
            {endPage < sellerList &&
              sellerList.transactions?.pagination?.totalPages && (
                <div
